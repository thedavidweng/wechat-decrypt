...
| 启动实时消息 Web UI | `python main.py` |
| **批量导出所有聊天** | `python export_all_chats.py` |
| **批量导出 + 语音转录** | `python export_all_chats.py --with-transcriptions` |
| 转录单个文件的语音 | `python transcribe_chat.py input.json [output.json]` |
| 查看当前数据状态 | `python main.py status` |
| 清理临时数据 | `python cleanup.py clean` |

## MCP Server (Claude AI) 使用案例
